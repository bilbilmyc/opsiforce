...
import type { ExternalServiceDefinition, StoredMessage, WebhookRequest } from './external-service-definition';
...
export class WebhookIngestService {
  ...
  private async storeForEnvironment(
    ...
    const rowIds = await this.store.store(envDirectory, definition.serviceName, rows);
