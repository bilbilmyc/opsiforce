...
import type { WhapiChatOption } from './whapi-channel.types';
import { WHAPI_GROUP_CHAT_SUFFIX, WHAPI_WEBHOOK_SECRET_HEADER } from './whapi.constants';
...
export class WhapiClient {
  ...
  async listChats(apiToken: string): Promise<WhapiChatOption[]> {
    const chats = await this.collectChats(apiToken);
    const groups = await this.collectGroups(apiToken);

    const byChatId = new Map<string, WhapiChatOption>();
      ...
      webhooks: [
        {
          url: webhookUrl,
          mode: 'body',
          headers: { [WHAPI_WEBHOOK_SECRET_HEADER]: webhookSecret },
  ...
  private async collectChats(apiToken: string): Promise<WhapiChatOption[]> {
    const collected: WhapiChatOption[] = [];
  ...
  private async collectGroups(apiToken: string): Promise<WhapiChatOption[]> {
    const collected: WhapiChatOption[] = [];
...
function chatKind(chatId: string): 'individual' | 'group' {
  return chatId.endsWith(WHAPI_GROUP_CHAT_SUFFIX) ? 'group' : 'individual';
}
