...
export class AppService implements OnModuleDestroy {
  ...
  private async fetchAppMeta(project: { id: string; podIp: string | null }): Promise<ProjectAppMeta | null> {
    ...
    try {
      ...
      this.logger.debug(`app-meta fetch failed for project ${project.id}: ${(err as Error).message}`);
      return null;
    }
  }
}
