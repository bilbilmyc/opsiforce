...
export class ScheduleService {
  ...
  constructor(@InjectQueue(SCHEDULE_QUEUE_NAME) private readonly queue: Queue<ScheduleJobData>) {}

  async upsert(projectId: string, projectEnvironmentId: string, tenantId: string, dto: CreateScheduleDto) {
  ...
  async findByTenant(tenantId: string, filter?: { environmentId?: string; projectEnvironmentId?: string }) {
  ...
  async findOneRaw(scheduleId: string) {
    const [schedule] = await db.select().from(projectSchedules).where(eq(projectSchedules.id, scheduleId));
  ...
  async removeByEnvironment(projectEnvironmentId: string, scheduleId: string) {
    const [schedule] = await db
      .select()
      .from(projectSchedules)
      .where(and(eq(projectSchedules.projectEnvironmentId, projectEnvironmentId), eq(projectSchedules.id, scheduleId)));
