...
export class PodCacheService implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  private async start(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `Pod informer failed to start: ${(err as Error).message}; retrying in ${RECONNECT_BACKOFF_MS}ms`
      );
  ...
  private handleError(err?: Error): void {
    ...
    this.logger.warn(
      `Pod informer error: ${err?.message ?? 'unknown error'}; reconnecting in ${RECONNECT_BACKOFF_MS}ms`
    );
    this.scheduleReconnect();
  }
