...
状态：2026-09-16 调整为 **一个聚合 App Builder + 统一 Node/Python/Go 环境**。技术栈属于项目内部实现，用户无需选择独立的语言 Agent。当前实现与验收边界见 [统一入口](../runtime/template-selection.md)。Java、Rust 后置，组合适配逐项验收。

## 目标与当前问题

用户指定 React、Vue、Next.js 或 Python、Go、Rust、Java 时，项目必须保留这个选择。工具安装成功不等于平台支持；一个正式模板必须同时支持文件浏览、开发、重启、生产构建、发布和持久化。

原 App Builder 的 agent.md 直接指定 React 页面、NestJS 模块和 Yarn；template/app/startup.sh 固定启动两个 Yarn 进程；entrypoint.sh 固定打开 app/data/app.db；发布写入 app/opsiforce.env.json。新项目现在从无框架工作区开始，由统一 Agent 按请求初始化。因此不应通过换一个默认框架、堆装语言或重写平台解决。
...
统一 App Builder 内部区分新建和已有项目：

1. 新建项目：根据用户明确指定的栈或需求选择内部适配；简要告知所用技术，直接实施。
2. 已有项目：先检查依赖、lockfile、工作目录、现有启动命令，生成可审阅的运行清单；不覆盖源码，不统一替换包管理器。基本导入能力随运行协议建设，复杂组合保持 experimental。

默认 UI 只提供聚合 App Builder，不增加语言/框架选择器。安装、构建、启动、端口与数据目录由平台约定和 Agent 处理；用户需要时可以查看或编辑运行清单。
...
| 对象 | 内容 | 示例 |
| --- | --- | --- |
| 运行工具配置 | 固定版本的编译器、包管理器、镜像摘要、缓存规则 | 首批统一 Node + Python + Go；Java/Rust 后续按需扩展 |
...
2026-09-16 进度：阶段 1a 已实现版本 1 启动清单及旧入口兼容；阶段 1b 新增 FastAPI/Go 内部脚手架；阶段 1c 已由独立语言入口调整为聚合 App Builder、无框架初始化与按需加载指南，详见 [模板选择](../runtime/template-selection.md)。前端组合尚未接入，不能视为下表阶段 2 已完成。
...
现有项目没有清单时标记为 legacy，读取原 startup.sh，不推断后就强制改造。只对新建空项目应用模板；后续升级采用带版本与冲突检查的显式迁移。后续源码迁移必须按实际技术栈筛选，不能仅凭同一个 App Builder 名称套用 NestJS 迁移。
