...
Each agent in `agent-config/agents.json` declares its own `poolSize`. Today only `app-builder` declares one (default `3`). Setting `poolSize: 0` disables the pool for that agent. Setting `POOL_SIZE_OVERRIDE` in the environment caps every agent's effective size — local dev uses `1`, and operators can set it to `0` as a global kill switch without rebuilding the image.
...
boot ─► sweep legacy warm pods (one-time)
     ...
     ─► recycle pending projects whose baked agent template is stale
     ─► replenish to target per agent

new project click + tenant uses global default model:
     ─► claim atomically: reserve a `pending` row via FOR UPDATE SKIP LOCKED,
        flip to `claiming`, PATCH Bifrost team's customer_id, write tenant_id
        onto VK / gateway rows, flip to `active`
     ─► return the ready project (~100–500ms)
     ─► async replenish for the claimed agent

new project click + tenant overrides default model:
     ─► skip the pool, fall through to the slow path (today's flow)
```
...
Pool projects are recycled when the baked workspace agent version is stale. The regular `AgentUpdateService` sweep skips unclaimed pool slots because its migration job would mount the same workspace as the live pool pod; claimed projects still migrate normally after they become `active`.
...
- The tenant has a custom default model in `tenant_agent_defaults` (because pool pods bake the global default model — see the OpenCode config file copied during init).
- The tenant has custom project/chat/backend budgets (because pool keys bake global budgets).
- The pool for the requested agent is empty (boot still warming up, recent burst of claims, or `poolSize=0`).
- The Bifrost team patch fails three times in a row (clean slots return to pending; ambiguous slots are destroyed; user uses the slow path).
...
- `agent-config/agents.json` — per-agent `poolSize`
