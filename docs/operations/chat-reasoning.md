...
- 每轮思考、工具执行和后台完成通知合并为一个「思考与执行过程」区域。执行中默认展开，结束后默认收起；用户手动选择优先。最终答复、错误和重试提示留在区域外。
...
- 69 项时间线投影、进度与过程分组测试通过；前端 Vite 构建通过。新增用例覆盖尾部答复保留、空思考转正文、多个用户回合、错误／重试、后台完成通知和历史加载时的身份保留。
- 完整 TypeScript 检查仍存在仓库已有错误；与本次修改前日志比较，没有新增诊断类型。
- 真实 FastAPI 对话包含 20 段思考、34 次工具调用，合并为一个可整体折叠的区域；最终答复单独可见。浏览器回归实际发送滚轮事件，验证区域内只滚动过程，区域外滚动对话，并检查手机宽度、双语及实时数据到达后保留手动折叠。
...
- 镜像：`sealos.hub:5000/opsiforce/opsiforce-frontend:chat-activity-20260915-1`
- 已核对运行 Pod 的 digest：`sha256:5be39d8d9b33a407733b0d0f9dcf2edfe2e1c59ad5d33ddcc6157daaae2bf9d8`
- 隔离构建目录：`/root/opsiforce/deploy/.state/chat-activity-20260915/source`
- 上级目录保留 `build.log` 和 `frontend-before.yaml`。
- 回滚镜像：`sealos.hub:5000/opsiforce/opsiforce-frontend:chat-reasoning-20260915-1`。
