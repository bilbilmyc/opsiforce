...
- Backend endpoint: `backend/src/project/download.controller.ts` and `download.service.ts` (part of the project module) — resolves the environment's workspace directory (the same way uploads do) and streams the file back with an attachment disposition.
- Frontend interceptor: `frontend/src/components/project/workspace-download-links.tsx`, mounted by the project chat tab.
- Agent guidance: the "Sharing files for download" section of `agent-config/agents/app-builder/agent.md`.
