...
The admin mount is **two-tiered**, because its route tables mix two very different audiences. A route that names an environment (`environments/:projectEnvironmentId/…`) is an organization's own business, and the dispatcher checks that the environment's project belongs to the caller's tenant before the handler runs. A route that names no environment reaches the platform-wide resource registry shared by every organization — registering a WhatsApp channel, rotating its secret, deleting it — so the dispatcher additionally demands `can_manage_external_service_resources`, a platform-operator permission that no ordinary organization admin should hold. The rule is derived from the route's own shape rather than declared per definition, which keeps the guarantee below intact.

The split has a consequence for anything an organization admin needs to *read* about shared resources, and WhatsApp shows the shape of the answer. Choosing a channel and browsing its chats are environment-scoped routes (`environments/:projectEnvironmentId/available-channels` and `…/channels/:channelId/chats`) that answer only for channels the caller's own organization already uses — the dispatcher hands every admin handler the resolved `tenantId`, so a definition can filter by tenant even though it never decides who is allowed in. Connecting a channel to an organization for the first time stays an operator act; from then on the organization wires its own environments without ever seeing another organization's channels or contacts.
...
- [Permissions](../organization/permissions.md) — `can_view_external_services_usage`, `can_manage_external_services`, `can_manage_external_service_resources`.
