...
Gated by `can_view_platform_storage`, a [platform-scope permission](../organization/permissions.md): it requires an authenticated human and nothing else — no Organization membership, no tenant context — and Pulumi never grants it as part of the default admin bundle. The view ignores the Organization selector entirely, because its whole point is the view across all of them ([ADR-0027](../adr/0027-platform-scope-routes-opt-out-of-tenant-resolution.md)).
