...
> How "New Project" feels instant: a pool of pre-baked, already-running projects waits to be claimed in a fast atomic flip instead of cold-starting. Read this to understand the claim flow and how to operate the pool.
...
- [Pod Lifecycle](pod-lifecycle.md) — how a claimed environment then behaves (and how the old warm pool was retired).
- [LLM Gateway](../gateways/llm-gateway.md) — the orphan Bifrost team a pool project carries until claim.
- Code: `backend/src/pool/project-pool.service.ts` (lifecycle: boot, claim, replenish, recycle, recovery); `backend/src/bifrost/bifrost.service.ts` (`createOrphanProjectResources`, `reassignTeamCustomer`); `backend/src/project/project.service.ts` (`create()` tries the pool, falls back to direct); `agent-config/agents.json` (per-agent `poolSize`).
