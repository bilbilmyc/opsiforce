...
**Finalize** is the existing `POST /api/projects/import`, now with a small JSON body naming the session. Because no session state is persisted, completeness is verified structurally at this point: the staged parts must be contiguous from zero and every part except the last must be exactly the chunk size. The parts are then stream-concatenated in order into the regular upload path and the unchanged import pipeline takes over. There is no whole-file hash — per-chunk digests plus strict lengths cover transport, and the zip's own CRC32s during unpack are the final backstop. The error contract splits on the status: a **400** means the staged bytes themselves were rejected and can never import, so the staging directory is deleted immediately; any other failure keeps the parts so the client can re-POST finalize without re-uploading anything — which is why assembly reads the parts rather than consuming them, and why the staging directory is only deleted after the import has started.
