...
- Builds on existing code: `backend/src/project/project-duplicate.processor.ts` (the async job + subtractive copy it mirrors), `backend/src/git/git.service.ts` (commit + local clone, full history), `backend/src/files/download.service.ts` (streaming from shared storage), `backend/src/pod/pod.template.ts` (why credentials and agent config are not carried).
