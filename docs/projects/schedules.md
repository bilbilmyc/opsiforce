...
> Per-environment recurring cron jobs an agent registers against its app — stored centrally, fired from inside the cluster (waking a suspended pod if needed), and recorded run-by-run. Read this to understand the per-environment ownership model.
...
- [Service Gateway](../gateways/service-gateway.md) — the per-project token schedules authenticate with.
- [Project Environments](environments.md) — why a schedule is owned by one environment and copied on publish.
- [Pod Lifecycle](../runtime/pod-lifecycle.md) — the wake-on-fire path (waking a suspended pod).
- [Permissions](../organization/permissions.md) — `can_manage_schedules` (enforced class-level on the admin controller).
- Code: `backend/src/schedule/` (agent-facing gateway-guarded endpoints, tenant-facing admin endpoints, BullMQ sync + worker); `projectSchedules` (keyed per environment) + `scheduleExecutions` in `backend/db/schema.ts`; agent skill `agent-config/agents/app-builder/skills/schedules/SKILL.md`.
