...
This is deliberate: reusing the projectId keeps every existing URL, pod name, on-disk directory, and satellite FK **byte-for-byte valid**. It turns what would be a high-risk data migration (rename CephFS directories, recreate pods, break live app URLs and catalog pins, issue redirects) into a metadata-only one — insert env rows and set `environmentId = projectId` (the value the satellite rows already hold).
...
- Partially superseded by ADR 0012: the public app hostname now appends an environment slug (`{envId}-{slug}`) and the hostname regexes widened accordingly. The env id remains the routing key, and its fixed 36-char uuid length became load-bearing for parsing the slug off the label.
