...
- The view surfaces drift (a pod whose DB status ≠ `active`; an env marked active with no pod) precisely because Kubernetes, not the DB, drives the row set. This is the same authority model as the rest of pod management (see [pod-management.md](../pod-management.md)), applied to a read view.
