...
Almost every route is organization-scoped: the global `TenantGuard` requires membership in the requested organization and stamps `request.tenantContext`, which services filter by. A handful of Admin views span *all* organizations, and for those "the current organization" is meaningless. Such a route is marked `@PlatformScope()` — the guard still demands `x-forwarded-groups` (it remains an authenticated-human route) but resolves no organization and stamps no context, so `@CurrentTenant()` must not be used on it. Membership in any organization is deliberately not required, so a pure-operator account works. Holding the route's `@RequirePermission` is the only gate. Two discovery endpoints are platform-scope for the same reason: permissions (`GET /api/permissions`) and the caller's organization list (`GET /api/tenants`). Both read only the groups header, and a membership-less operator has to be able to learn what they hold — and that they belong to nothing — before any gated view can appear.

That answer also shapes the shell. Because the API client turns any 403 into a redirect to the permission-denied page, a single organization-scoped boot query would strand a platform-only operator before their view ever rendered. So the sidebar and the home route mount their organization-scoped parts — the project list, the create menu, the composer — only once the organization list comes back non-empty; the user chip reads its name from the proxy's userinfo rather than a per-organization user record. An operator with no organization lands on their first permitted platform view instead of the composer, and sees the rest of the shell empty rather than broken. See [ADR-0027](../adr/0027-platform-scope-routes-opt-out-of-tenant-resolution.md).
