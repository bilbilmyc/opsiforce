...
import AttributeSheet, { type CopiedCapabilityTemplate } from "./attributeSheet";
...
export default function AttributesTab({ hasAccess }: AttributesTabProps) {
	...
	const [copiedTemplate, setCopiedTemplate] = useState<CopiedCapabilityTemplate>();
	...
	return (
		<>
			{editing && (
				<AttributeSheet copiedTemplate={copiedTemplate} onCopy={setCopiedTemplate} model={editing} onClose={() => setEditing(null)} />
			)}
			...
			<div className="flex min-h-0 w-full grow flex-col overflow-hidden">
				<div className="mb-4 flex shrink-0 items-center justify-between">
					<div>
						...
						<p className="text-muted-foreground text-sm">选择模型后配置容量和功能开关，已提供默认模板。</p>
					</div>
				</div>
					...
					<Table containerClassName="h-full overflow-y-auto overflow-x-hidden" className="table-fixed">
						<TableHeader className="bg-muted sticky top-0 z-20">
							<TableRow className="hover:bg-transparent">
								<TableHead className="w-[116px] font-medium">渠道</TableHead>
								<TableHead className="font-medium">模型</TableHead>
								<TableHead className="w-[72px] px-2 text-right font-medium">输入价格</TableHead>
								<TableHead className="w-[76px] px-2 text-right font-medium">输出价格</TableHead>
								<TableHead className="w-[86px] px-2 text-right font-medium">缓存写入</TableHead>
								<TableHead className="w-[80px] px-2 text-right font-medium">缓存读取</TableHead>
								<TableHead className="font-medium">模型说明</TableHead>
								<TableHead className="w-[68px] font-medium">配置</TableHead>
								...
								<TableRow>
									<TableCell colSpan={9} className="h-24 text-center">
										<span className="text-muted-foreground text-sm">
											{!debouncedSearch && !providerFilter ? "暂无模型，请先添加渠道。" : "未找到匹配的模型。"}
										</span>
									</TableCell>
								</TableRow>
							) : (
										...
										<TableRow key={`${m.provider}|${m.name}`} data-testid={`model-catalog-row-${testKey}`}>
											...
											<TableCell className="py-3">
												{extraKeys.length === 0 ? (
													...
													<Badge variant="secondary">{extraKeys.length} 项</Badge>
												)}
											</TableCell>
				...
				{totalCount > 0 && (
					<div className="flex shrink-0 items-center justify-between text-xs" data-testid="model-catalog-pagination">
						<div className="text-muted-foreground">
							{(offset + 1).toLocaleString()}–{Math.min(offset + PAGE_SIZE, totalCount).toLocaleString()} / {totalCount.toLocaleString()} 条
						</div>
