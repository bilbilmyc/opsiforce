...
export default function ModelCatalogTable({
	...
	const summaryCards = [
		{ label: "渠道总数", value: totalProviders.toLocaleString() },
		{ label: "模型总数", value: totalModels.toLocaleString() },
		{ label: "近一天请求数", value: totalRequests24h.toLocaleString() },
		{ label: "近一天费用", value: formatCost(totalCost24h) },
	];
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground text-sm">查看已配置渠道、模型及调用情况。</p>
				</div>
				<Select
				...
				<Table className="table-fixed">
					...
					<TableHeader>
						<TableRow>
							...
							<TableHead className="text-right">近一天调用量</TableHead>
							<TableHead className="text-right">近一天费用</TableHead>
						</TableRow>
					</TableHeader>
					<TableBody>
						{rows.length === 0 ? (
							<TableRow>
								<TableCell colSpan={4} className="h-24 text-center">
									<span className="text-muted-foreground text-sm">未找到匹配的渠道。</span>
								</TableCell>
							</TableRow>
						) : (
							rows.map((row) => (
