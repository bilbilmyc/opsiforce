...
export default function ModelProviderKeysTableView({ provider, className, headerActions, isKeyless }: Props) {
	...
	const entityLabel = isVLLM ? "模型" : isOllamaOrSGL ? "服务器" : "密钥";
	const entityLabelPlural = isVLLM ? "模型" : isOllamaOrSGL ? "服务器" : "密钥";
		...
		<div className={cn("w-full", className)}>
			...
			<CardHeader className="mb-4 px-0">
				<CardTitle className="flex items-center justify-between">
					<div className="flex items-center gap-2">已配置{entityLabelPlural}</div>
							...
							<Tooltip>
								<TooltipTrigger asChild>
									...
									>
										...
										{isRefreshingProvider ? "Refreshing..." : "刷新模型列表"}
									</Button>
								</TooltipTrigger>
				...
				<div className="flex w-full flex-col gap-2 rounded-sm border">
					<Table className="w-full table-fixed" data-testid="keys-table">
						...
						<TableHeader className="w-full">
							<TableRow>
								<TableHead>{isVLLM ? "Model" : isOllamaOrSGL ? "Server" : "API 密钥"}</TableHead>
						...
						<TableBody>
							...
							{keys.map((key) => {
								const isKey已启用 = key.enabled ?? true;
								return (
									<TableRow
										key={key.id}
