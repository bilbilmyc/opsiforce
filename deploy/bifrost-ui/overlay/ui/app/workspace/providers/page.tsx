import QuickChannelSheet from "./dialogs/quickChannelSheet";
	...
	if (configuredProviders.length === 0) {
		return (
			<div className="mx-auto w-full max-w-7xl">
				<ProvidersEmptyState
					addProviderDropdown={
						<AddProviderDropdown
							...
							onQuickChannel={() => setShowQuickChannel(true)}
				...
				/>
				<QuickChannelSheet
					show={showQuickChannel}
					onClose={() => setShowQuickChannel(false)}
					onSave={(name) => {
						setShowQuickChannel(false);
						setProvider(name);
						toast.success("渠道和模型已保存，请到模型管理核对能力配置", { position: "top-center" });
					}}
				/>
				...
				<TooltipProvider>
					<div className="custom-scrollbar flex-1 overflow-y-auto">
						<div className="rounded-md bg-zinc-50/50 p-4 dark:bg-zinc-800/20">
							{/* 已配置渠道 (standard with keys + custom) */}
							{configuredProviders.length > 0 && (
								<div className="mb-4">
									<div className="text-muted-foreground mb-2 text-xs font-medium">已配置渠道</div>
