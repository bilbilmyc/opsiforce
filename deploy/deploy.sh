...
BIFROST_CONSOLE_URL=${BIFROST_CONSOLE_URL:-}
if [[ -n "$BIFROST_CONSOLE_URL" && ! "$BIFROST_CONSOLE_URL" =~ ^https?://[a-zA-Z0-9._:/?=\&%+~-]+$ ]]; then
  echo 'BIFROST_CONSOLE_URL 必须是有效的 HTTP/HTTPS 管理页面地址' >&2; exit 1
fi
