---
name: send-email
description: Informational skill — the platform does NOT currently support sending emails. Use when the user asks for email features (notifications, reports, welcome emails, password resets, etc.) to explain this limitation.
---
...
<!--
The previous version of this skill documented sending email through the service gateway
(`POST $SERVICE_GATEWAY_URL` with `{ service: "email", payload: { to, subject, html } }`)
and included an email HTML styling guide. That flow is currently disabled at the backend
gateway level. If email support is re-enabled in the future, restore this skill from git
history.
-->
