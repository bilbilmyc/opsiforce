  ...
  return (
    <div class="col-span-full grid grid-cols-subgrid items-center rounded-md border border-dashed border-border/80 px-2.5 py-2">
      <span class="flex min-w-0 items-center gap-1.5">
        ...
        <span class="truncate text-xs font-semibold text-foreground" title={environmentDisplayName(props.target)}>
          {environmentDisplayName(props.target)}
        </span>
      </span>
      <EnvAppLinkButtons appUrl="" enabled={false} disabledReason={t("Publish first — no app is running here yet")} />
      <span class="truncate text-xs text-muted-foreground">{t("Not published yet")}</span>
      <Show when={props.canPublish && props.hasApp}>
        ...
        >
          <Rocket class="h-3.5 w-3.5" />{t("Publish")}</Button>
      </Show>
    </div>
  );
}
