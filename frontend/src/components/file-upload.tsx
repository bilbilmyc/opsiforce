...
const FileUpload: Component<{ projectId: string; environmentId: string }> = (props) => {
  ...
  async function uploadFiles(files: FileList | File[], resetInput?: () => void) {
  ...
  function sendUploadStreaming(files: FileList | File[], totalData: number): Promise<UploadResult> {
  ...
  function sendUploadXhr(files: FileList | File[], totalData: number): Promise<UploadResult> {
  ...
  function sendUpload(files: FileList | File[], totalData: number): Promise<UploadResult> {
    return supportsRequestStreams ? sendUploadStreaming(files, totalData) : sendUploadXhr(files, totalData);
  }
          ...
          fallback={
            <div class="flex flex-col gap-1.5">
              <div class="flex h-7 items-center gap-3">
                ...
                <span class="text-xs text-muted-foreground tabular-nums w-9 text-right">{percent()}%</span>
                <button
              ...
              <div class="flex items-center justify-between gap-3 text-[11px] text-muted-foreground">
                ...
                <span class="tabular-nums shrink-0">
                  <Show when={totalBytes() > 0} fallback={`${preparedFiles()} / ${totalFiles()} files`}>
