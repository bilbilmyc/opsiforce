...
import { useConfigureWhapiWebhook, type WhapiChannel } from '~/api/whapi-channels';
...
export function ConfigureWebhookDialog(props: {
  ...
  const configure = useConfigureWhapiWebhook();
  ...
  const submit = async () => {
    ...
    try {
      const result = await configure.mutateAsync({ id: channel.id, webhookUrl: webhookUrl().trim() });
      ...
      <DialogContent class="max-w-md">
        ...
        <div class="mt-4 space-y-3">
          <LabeledTextField
            ...
            placeholder="https://opsiforce.example.com/api/webhooks/whatsapp"
            mono
            autofocus
            onEnter={submit}
            error={webhookUrl().trim() && !canSubmit() ? 'Must be an http(s) URL' : null}
            hint="Must be reachable from Whapi — use a tunnel when testing against a local backend."
          />
