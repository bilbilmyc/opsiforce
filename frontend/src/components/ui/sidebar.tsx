...
export function SidebarHeader(props: ParentProps<{ class?: string }>) {
  ...
  return <div class={cn("flex flex-col gap-2 p-2 shrink-0", local.class)} {...rest}>{local.children}</div>
}

export function SidebarContent(props: ParentProps<{ class?: string }>) {
  const [local, rest] = splitProps(props, ["class", "children"])
  return <div class={cn("flex-1 overflow-y-auto overflow-x-hidden sidebar-scroll", local.class)} {...rest}>{local.children}</div>
}
...
export function SidebarGroup(props: ParentProps<{ class?: string }>) {
  const [local, rest] = splitProps(props, ["class", "children"])
  return <div class={cn("flex flex-col gap-1 px-2 py-2", local.class)} {...rest}>{local.children}</div>
}
...
export function SidebarGroupAction(props: ParentProps<{
  class?: string
  onClick?: () => void
  disabled?: boolean
  title?: string
...
export function SidebarGroupContent(props: ParentProps<{ class?: string }>) {
  const [local, rest] = splitProps(props, ["class", "children"])
  return <div class={cn("flex flex-col gap-0.5", local.class)} {...rest}>{local.children}</div>
}

export function SidebarMenu(props: ParentProps<{ class?: string }>) {
  const [local, rest] = splitProps(props, ["class", "children"])
  return <ul class={cn("flex flex-col gap-0.5", local.class)} {...rest}>{local.children}</ul>
}

export function SidebarMenuItem(props: ParentProps<{ class?: string }>) {
  const [local, rest] = splitProps(props, ["class", "children"])
  return <li class={cn("list-none", local.class)} {...rest}>{local.children}</li>
}

export function SidebarMenuButton(props: ParentProps<{
  class?: string
  isActive?: boolean
