...
export function ToolbarButton(props: ToolbarButtonProps) {
  const [local, rest] = splitProps(props, ['class', 'size', 'tone', 'tooltip', 'children', 'aria-label']);

  const buttonClass = () => cn(toolbarButtonVariants({ size: local.size, tone: local.tone }), local.class);
  ...
  return (
    ...
    >
      <Tooltip>
        <TooltipTrigger as="button" class={buttonClass()} aria-label={label()} {...rest}>
          {local.children}
        </TooltipTrigger>
