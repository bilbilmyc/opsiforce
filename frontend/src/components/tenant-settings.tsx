...
export default function TenantSettings(props: {
  ...
  const defaultTab = createMemo<TenantSettingsTab>(() =>
    canManage() ? "integrations" : "environments",
  )
  ...
  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent>
        ...
        <Tabs
          value={activeTab()}
          onChange={(v) => setActiveTab(v as TenantSettingsTab)}
          class="mt-4"
        >
          <TabsList>
            <Show when={canManage()}>
                ...
                <div class="space-y-2">
                  ...
                  <Show when={savedMakaraTenantName() && !savedIsInList()}>
                    <div class="flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-foreground">
                      ...
                      <div>
                        Currently mapped to{" "}
                        <span class="font-medium">{savedMakaraTenantName()}</span> — not in your
